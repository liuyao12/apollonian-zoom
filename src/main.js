...
function resize(){canvas.width=innerWidth-150;canvas.height=innerHeight;update();draw();}
addEventListener('resize',resize);
function draw(){ctx.fillStyle='white';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.strokeStyle='black';ctx.fillStyle='black';ctx.textAlign='center';ctx.textBaseline='middle';
for(const e of circles){const c=toFloat(e);const x=offsetX+c.x*zoom,y=offsetY-c.y*zoom,r=c.r*zoom;if(!Number.isFinite(x+y+r))continue;if(x+r<0||x-r>canvas.width||y+r<0||y-r>canvas.height)continue;ctx.beginPath();ctx.arc(x,y,r,0,2*Math.PI);ctx.stroke();if(c.b<0n){ctx.font=Math.max(18,r*.25)+'px sans-serif';ctx.fillText('-1',x+r/Math.SQRT2+Math.max(30,r*.2),y-r/Math.SQRT2-Math.max(30,r*.2));}else if(r>10){ctx.font=Math.max(9,Math.min(r*.4,40))+'px sans-serif';ctx.fillText(c.b.toString(),x,y);}}
}
