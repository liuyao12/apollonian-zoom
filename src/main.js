...
function drawThumbnail(canvasEl,config){
 ...
 const cs=generateVisible(config,vp,cam.zoom);
...
function renderCards(){
 ...
 for(const name of Object.keys(presets)){
  ...
  d.className='card'+(name===current?' selected':'');
  const t=document.createElement('canvas');
  t.width=130;t.height=100;t.className='thumb';
...
function draw(){
 ...
 for(const e of circles){
  ...
  if(c.b<0n){ctx.font=Math.max(18,r*.25)+'px sans-serif';const d=r+Math.max(30,r*.2);ctx.fillText(c.b.toString(),x+d/Math.SQRT2,y-d/Math.SQRT2);}
