...
export function reflect(config,i){
 ...
 for(let j=0;j<4;j++) if(j!==i){
  b+=config[j].b;
  bx=add(bx,config[j].bx);
  by=add(by,config[j].by);
 }
 out[i]={
  b:2n*b-config[i].b,
  bx:sub(mulInt(bx,2n),config[i].bx),
  by:sub(mulInt(by,2n),config[i].by)
 };
 return out;
}
...
function visible(c,v,m){const x=toFloat(c);return !(x.x+x.r<v.left-m||x.x-x.r>v.right+m||x.y+x.r<v.bottom-m||x.y-x.r>v.top+m);}
